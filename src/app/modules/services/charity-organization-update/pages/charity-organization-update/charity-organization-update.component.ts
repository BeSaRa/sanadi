...
export class CharityOrganizationUpdateComponent
  ...
  _updateForm(model: CharityOrganizationUpdate | undefined): void {
    ...
    if(this.operation == this.operationTypes.CREATE) {
      ...
      this.model.charityBranchList = this.model.charityBranchList?.map(cb => {
        return {
          ...cb,
          itemId: cb.id.toString(),
          branchContactOfficer: cb.branchContactOfficer.map(bco => {
            return {
              ...bco,
              itemId: bco.id.toString()
            } as OrganizationOfficer;
          }),
          branchContactOfficerList: cb.branchContactOfficerList.map(bco => {
            return {
              ...bco,
              itemId: bco.id.toString()
            } as OrganizationOfficer;
      ...
      this.model.realBeneficiaryList = this.model.realBeneficiaryList?.map(rb => {
        return {
          ...rb,
          itemId: rb.id.toString()
        } as RealBeneficiary;
      });
      this.model.boardMemberList = this.model.boardMemberList?.map(bm => {
        return {
          ...bm,
          itemId: bm.id.toString()
        } as OrgMember;
      });
      this.model.authorizedSignatoryMemberList = this.model.authorizedSignatoryMemberList?.map(asm => {
        return {
          ...asm,
          itemId: asm.id.toString()
        } as OrgMember;
      });
      this.model.founderMemberList = this.model.founderMemberList?.map(fm => {
        return {
          ...fm,
          itemId: fm.id.toString()
        } as OrgMember;
      });
      this.model.generalAssemblyMemberList = this.model.generalAssemblyMemberList?.map(gam => {
        return {
          ...gam,
          itemId: gam.id.toString()
        } as OrgMember;
      });
      this.model.currentExecutiveManagementList = this.model.currentExecutiveManagementList?.map(cem => {
        return {
          ...cem,
          itemId: cem.id.toString()
        } as OrgExecutiveMember;
      });
      this.model.riskReportList = this.model.riskReportList?.map(rr => {
        return {
          ...rr,
          itemId: rr.id.toString()
        } as CharityReport;
      });
      this.model.coordinationSupportReport = this.model.coordinationSupportReport?.map(csr => {
        return {
          ...csr,
          itemId: csr.id.toString()
        } as CharityReport;
      });
      this.model.incomingReportList = this.model.incomingReportList?.map(ir => {
        return {
          ...ir,
          itemId: ir.id.toString()
        } as CharityReport;
      });
      this.model.incomingDecisionList = this.model.incomingDecisionList?.map(idItem => {
        return {
          ...idItem,
          itemId: idItem.id.toString()
        } as CharityDecision;
      });
      this.model.outgoingDecisionList = this.model.outgoingDecisionList?.map(od => {
        return {
          ...od,
          itemId: od.id.toString()
        } as CharityDecision;
      });
      this.model.complianceOfficerList = this.model.complianceOfficerList?.map(co => {
        return {
          ...co,
          itemId: co.id.toString()
        } as OrganizationOfficer;
      });
      this.model.wFClassificationList = this.model.wFClassificationList?.map(wfC => {
        return {
          ...wfC,
          itemId: wfC.id.toString()
        } as ForeignAidClassification;
      });
      this.model.charityContactOfficerList = this.model.charityContactOfficerList?.map(cco => {
        return {
          ...cco,
          itemId: cco.id.toString()
        } as OrganizationOfficer;
      });
