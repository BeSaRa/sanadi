import { Component, OnInit, AfterViewInit, OnDestroy, EventEmitter, ViewChild, Output, Input } from '@angular/core';
...
import { CollectionItemClone } from '@app/models/collection-item-clone';
...
@Component({
  selector: 'collection-item-clone',
  templateUrl: './collection-item-clone.component.html',
  styleUrls: ['./collection-item-clone.component.scss']
})
export class CollectionItemCloneComponent extends HasAttachmentHandlerDirective implements OnInit, AfterViewInit, OnDestroy{
  ...
  @Input()
  model!: CollectionApprovalClone;

  @Input()
  formProperties: Record<string, () => Observable<any>> = {}
  destroy$: Subject<any> = new Subject<any>();
  add$: Subject<any> = new Subject<any>();
  edit$: Subject<{ item: CollectionItemClone, index: number }> = new Subject<{ item: CollectionItemClone, index: number }>();
  view$: Subject<{ item: CollectionItemClone, index: number }> = new Subject<{ item: CollectionItemClone, index: number }>();
  remove$: Subject<{ item: CollectionItemClone, index: number }> = new Subject<{ item: CollectionItemClone; index: number }>();
  save$: Subject<null> = new Subject<null>();

  editIndex: number | undefined = undefined;

  item?: CollectionItemClone;
  ...
  actions: IMenuItem<CollectionItemClone>[] = [
    // view
    {
      type: 'action',
      label: 'view',
      icon: ActionIconsEnum.VIEW,
      onClick: (item: CollectionItemClone, index: number) => this.view$.next({ item: item, index: index }),
      show: (_item: CollectionItemClone) => !this.approvalMode && this.readOnly
    },
    // edit
    {
      type: 'action',
      ...
      onClick: (item: CollectionItemClone, index: number) => this.edit$.next({ item: item, index: index }),
      show: (_item: CollectionItemClone) => !this.approvalMode,
      disabled: (_item: CollectionItemClone) => this.readOnly
    },
    // delete
    {
      type: 'action',
      ...
      onClick: (item: CollectionItemClone, index: number) => this.remove$.next({ item: item, index: index }),
      show: (_item: CollectionItemClone) => !this.approvalMode,
      disabled: (_item: CollectionItemClone) => this.readOnly
    },
    // edit approval info (if approval mode)
    {
      ...
      icon: ActionIconsEnum.EDIT,
      onClick: (item: CollectionItemClone, index: number) => this.approval.emit({ item: item, index: index }),
      show: (_item: CollectionItemClone) => this.approvalMode,
      disabled: (_item: CollectionItemClone) => this.readOnly
    },
  ];
  ...
  @Output()
  approval: EventEmitter<{ item: CollectionItemClone, index: number }> = new EventEmitter<{ item: CollectionItemClone; index: number }>();
  ...
  private listenToAdd() {
    this.add$
      ...
      .pipe(tap(_ => {
        this.item = new CollectionItemClone().clone<CollectionItemClone>({
  ...
  private buildForm(): void {
    this.form = this.fb.group((new CollectionItemClone().buildForm(true)));
  ...
  private listenToSave(): void {
    this.save$
      ...
      .subscribe(() => {
        this.processSave(new CollectionItemClone().clone({
          ...this.item,
          ...this.form.value,
  ...
  openLocationMap(item: CollectionItemClone) {
    item.openMap(true);
  }
    ...
    this.licenseSearch$
      ...
      .pipe(exhaustMap((serial) => {
        return this.licenseService
          .collectionSearch<CollectionApprovalClone>({
            fullSerial: serial,
            requestClassification: this.model.requestClassification,
            licenseDurationType: this.model.licenseDurationType
          });
  ...
  viewOldLicense(item: CollectionItemClone): void {
  ...
  viewGeneratedLicense(item: CollectionItemClone): void {
