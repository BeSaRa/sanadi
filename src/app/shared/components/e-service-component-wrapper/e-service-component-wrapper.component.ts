    ...
    this.userInboxActions = [
      ...
      // send to multi department
      {
        ...
        show: (item: CaseModel<any, any>) => {
          return item.getResponses().includes(WFResponseType.INTERNAL_PROJECT_SEND_TO_MULTI_DEPARTMENTS)
            ...
            || item.getResponses().includes(WFResponseType.FINAL_EXTERNAL_OFFICE_SEND_TO_MULTI_DEPARTMENTS)
            || item.getResponses().includes(WFResponseType.PARTNER_APPROVAL_SEND_TO_MULTI_DEPARTMENTS)
      ...
      // download final report
      {
        ...
        show: (item: CaseModel<any, any>) => {
          ...
          return ((item.getCaseType() === CaseTypes.GENERAL_ASSOCIATION_MEETING_ATTENDANCE && this.employeeService.isInternalUser())
