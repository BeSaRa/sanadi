  ...
  implements IModelInterceptor<PartnerApproval>
{
  receive(model: PartnerApproval): PartnerApproval {
    ...
    model.requestClassificationInfo = AdminResult.createInstance(
      model.requestClassificationInfo
    );

    model.managerDecisionInfo = AdminResult.createInstance(
      model.managerDecisionInfo
    );
    model.specialistDecisionInfo = AdminResult.createInstance(
      model.specialistDecisionInfo
    );
    model.chiefDecisionInfo = AdminResult.createInstance(
      model.chiefDecisionInfo
    );
    model.reviewerDepartmentDecisionInfo = AdminResult.createInstance(
      model.reviewerDepartmentDecisionInfo
    );
    model.licenseStatusInfo = AdminResult.createInstance(
      model.licenseStatusInfo
    );
    ...
    model.managementCouncilList = model.managementCouncilList.map((x) =>
      service.managementCouncilInterceptor.receive(
        new ManagementCouncil().clone(x)
      )
    );
    model.executiveManagementList = model.executiveManagementList.map((x) =>
      service.executiveManagementInterceptor.receive(
        new ExecutiveManagement().clone(x)
      )
    );
    model.targetGroupList = model.targetGroupList.map((x) =>
      service.targetGroupInterceptor.receive(new TargetGroup().clone(x))
    );
    model.contactOfficerList = model.contactOfficerList.map((x) =>
      service.contactOfficerInterceptor.receive(new ContactOfficer().clone(x))
    );
    model.approvalReasonList = model.approvalReasonList.map((x) =>
      service.approvalReasonInterceptor.receive(new ApprovalReason().clone(x))
    );
    model.workAreaObjectList = model.workAreaObjectList.map((x) =>
      service.workAreaInterceptor.receive(new WorkArea().clone(x))
    );
    model.commercialActivitiesList = model.commercialActivitiesList.map((x) =>
      service.commercialActivityInterceptor.receive(
