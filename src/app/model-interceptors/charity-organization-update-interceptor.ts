...
export class CharityOrganizationUpdateInterceptor implements IModelInterceptor<CharityOrganizationUpdate> {
  ...
  send(model: Partial<CharityOrganizationUpdate>): Partial<CharityOrganizationUpdate> {
    ...
    model.charityBranchList = model.charityBranchList?.map(e => charityBranchInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as CharityBranch);
    model.realBeneficiaryList = model.realBeneficiaryList?.map(e => realBeneficiaryInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as RealBeneficiary);
    model.boardMemberList = model.boardMemberList?.map(e => membersInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as OrgMember);
    model.authorizedSignatoryMemberList = model.authorizedSignatoryMemberList?.map(e => membersInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as OrgMember);
    model.founderMemberList = model.founderMemberList?.map(e => membersInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as OrgMember);
    model.generalAssemblyMemberList = model.generalAssemblyMemberList?.map(e => membersInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as OrgMember);
    model.currentExecutiveManagementList = model.currentExecutiveManagementList?.map(e => executiveMemberInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as OrgExecutiveMember);
    model.riskReportList = model.riskReportList?.map(e => charityReportInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as CharityReport);
    model.coordinationSupportReport = model.coordinationSupportReport?.map(e => charityReportInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as CharityReport);
    model.incomingReportList = model.incomingReportList?.map(e => charityReportInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as CharityReport);
    model.incomingDecisionList = model.incomingDecisionList?.map(e => charityDecisionInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as CharityDecision);
    model.outgoingDecisionList = model.outgoingDecisionList?.map(e => charityDecisionInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as CharityDecision);
    model.complianceOfficerList = model.complianceOfficerList?.map(e => organizationOfficer.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as OrganizationOfficer);
    model.wFClassificationList = model.wFClassificationList?.map(e => foreignAidClassificationInterceptor.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as ForeignAidClassification);
    model.charityContactOfficerList = model.charityContactOfficerList?.map(e => organizationOfficer.send({ ...e, itemId: !e.itemId ? e.id?.toString() : e.itemId }) as OrganizationOfficer);
