...
const beneficiarySearchLogCriteriaInterceptor =
  new BeneficiarySearchLogCriteriaInterceptor();
...
export class BeneficiaryService extends CrudGenericService<Beneficiary> {
  ...
  private _loadByCriteria(
    criteria: Partial<IBeneficiaryCriteria>
  ): Observable<Beneficiary[]> {
    return this.http.post<Beneficiary[]>(
      this.urlService.URLS.BENEFICIARY + '/criteria',
      criteria
    );
  }
  ...
  @HasInterception
  createWithValidate(
    @InterceptParam() beneficiary: Partial<Beneficiary>,
    validate: boolean = true,
    validateMoph: boolean = true
  ): Observable<Pair<BeneficiarySaveStatus, Beneficiary>> {
    delete beneficiary.id;
    let params = new HttpParams({
      fromObject: {
        'with-check': validate + '',
        'with-moph-check': validateMoph,
      },
    });
    return this.http
      .post<Pair<BeneficiarySaveStatus, Beneficiary>>(
        this._getServiceURL() + '/validate-save',
        beneficiary,
        {
          params: params,
        }
      ...
      .pipe(
        map((response: any) => {
          response.rs.second = response.rs.second
            ? this._getReceiveInterceptor()(
                new Beneficiary().clone(response.rs.second)
              )
            : response.rs.second;
          return response.rs;
        })
  ...
  openSelectBeneficiaryDialog(list: Beneficiary[]): DialogRef {
    return this.dialogService.show<Beneficiary[]>(
  ...
  loadBeneficiaryAuditData(requestId: number): Observable<SanadiAuditResult[]> {
    return this.http
      .get<IDefaultResponse<SanadiAuditResult[]>>(
  ...
  @CastResponse(undefined)
  private _loadBeneficiaryAuditDetails(
    auditId: number
  ): Observable<Beneficiary> {
  ...
  private _loadBeneficiaryLogByCriteria(
    @InterceptParam(beneficiarySearchLogCriteriaInterceptor.send)
    criteria: Partial<IBeneficiarySearchLogCriteria>
  ): Observable<BeneficiarySearchLog[]> {
    criteria.limit = this.configurationService.CONFIG.BENEFICIARY_AUDIT_LIMIT;
    return this.http.post<BeneficiarySearchLog[]>(
      this._getServiceURL() + '/search/audit/criteria',
      criteria
    );
  }

  loadBeneficiaryLogByCriteria(
    criteria: Partial<IBeneficiarySearchLogCriteria>
  ): Observable<BeneficiarySearchLog[]> {
  ...
  loadGDXMOPHMortality(criteria: IGdxCriteria) {
    return this.http.post<GdxMophResponse[]>(
      this._getGDXServiceURL() + '/moph/mortality',
      criteria
    );
  }
  ...
  beneficiaryStatus(
    @InterceptParam() beneficiary: Partial<Beneficiary>
  ): Observable<Pagination<SubventionRequestAid[]>> {
    return this.http.post<Pagination<SubventionRequestAid[]>>(
      this._getServiceURL() + '/beneficiary-status',
      beneficiary
    );
  }
  ...
  loadGDXIntegrationData(criteria: IGdxCriteria) {
    return this.http.post<GdxServiceLog[]>(
      this._getGDXServiceURL() + '/audit',
      criteria
    );
  }
  ...
  addMOJInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxMojResponse[]>(
      this._getGDXServiceURL() + '/moj/real-estate',
      criteria
    );
  }
  ...
  addMOCIInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxMociResponse[]>(
      this._getGDXServiceURL() + '/moci/commercial-record',
      criteria
    );
  }
  ...
  addMAWAREDInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxMawaredResponse[]>(
      this._getGDXServiceURL() + '/mawared/last-salary',
      criteria
    );
  }
  ...
  addGarsiaInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxGarsiaPensionResponse[]>(
      this._getGDXServiceURL() + '/garsia/pension',
      criteria
    );
  }
  ...
  addIzzabInquiry(criteria: IGdxCriteria): Observable<any> {
    return this.http
      .post<any>(this._getGDXServiceURL() + '/izzab-status', criteria)
  ...
  addKahramaaInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxKahramaaResponse[]>(
      this._getGDXServiceURL() + '/kaharmaa-outstanding',
      criteria
    );
  }
  ...
  addMOLInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxMolPayrollResponse[]>(
      this._getGDXServiceURL() + '/mol-payroll',
      criteria
    );
  }
  ...
  addSJCInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxSjcMaritalStatusResponse[]>(
      this._getGDXServiceURL() + '/sjc/marital-status',
      criteria
    );
  }
  ...
  addMOEPendingInstallments(criteria: IGdxCriteria) {
    return this.http.post<GdxMoeResponse[]>(
      this._getGDXServiceURL() + '/moe-pending-installments',
      criteria
    );
  }
  ...
  addMMELeasedContract(criteria: IGdxCriteria) {
    return this.http.post<GdxMmeResponse[]>(
      this._getGDXServiceURL() + '/mme-leased-contracts',
      criteria
    );
  }
  ...
  addQatarCharityInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxQatarCharityResponse[]>(
      this._getGDXServiceURL() + '/qc-historical-data',
      criteria
    );
  }
  ...
  addEidCharitableFoundationInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxEidCharitableFoundationResponse[]>(
      this._getGDXServiceURL() + '/eid-historical-data',
      criteria
    );
  }
  ...
  addQatarRedCrescentInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxQatarRedCrescentResponse[]>(
      this._getGDXServiceURL() + '/qrcs-historical-data',
      criteria
    );
  }
  ...
  addQCBIbanInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxQcbIbanResponse[]>(
      this._getGDXServiceURL() + '/qcb/iban',
      criteria
    );
  }
  ...
  addHousingBenStatusInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxMsdfHousingResponse[]>(
      this._getGDXServiceURL() + '/msdf/housing-beneficiary-status',
      criteria
    );
  }
  ...
  addSecurityBenStatusInquiry(criteria: IGdxCriteria) {
    return this.http.post<GdxMsdfSecurityResponse[]>(
      this._getGDXServiceURL() + '/msdf/security-beneficiary-status',
      criteria
    );
  }
  ...
  addQCBInquiryReport(
    criteria: IGdxCriteria,
    requests: SubventionRequestAid[] = []
  ): Observable<GdxQCBResponse[]> {
    return this.dialogService
      .show(QcbFilesPopupComponent, {
        title: 'file_uploader',
        extensions: [
  ...
  addPersonalInfoInquiry(qId: string, expiryDate: string) {
    ...
    return this.http.post<GdxMoiPersonal>(
      this._getGDXServiceURL() + '/moi/person-info',
      criteria
    );
  }
  ...
  addAddressInfoInquiry(qId: string, expiryDate: string) {
    ...
    return this.http.post<GdxMoiAddress>(
      this._getGDXServiceURL() + '/moi/address-info',
      criteria
    );
  }

  getBeneficiaryFromMoiData(
    criteria: Partial<IBeneficiaryCriteria>
  ): Observable<Beneficiary[]> {
    const genders = this.lookupService.listByCategory.Gender;
    const { benPrimaryIdNumber, expiryDate } = criteria;
    const expiryDateString = DateUtils.changeDateFromDatepicker(
      expiryDate as unknown as IMyDateModel
    )?.toLocaleDateString('en-CA');
    return forkJoin([
      this.addPersonalInfoInquiry(benPrimaryIdNumber!, expiryDateString!),
      this.addAddressInfoInquiry(benPrimaryIdNumber!, expiryDateString!),
    ]).pipe(
      map(([person, address]) => {
        const gender = genders.find(
          (gender) =>
            gender.enName.toLowerCase() === person.sex.trim().toLowerCase()
        )?.lookupKey;
  ...
  downloadQCBReport(gdxServiceLogId: number) {
    return this.http
      .get(
        this._getGDXServiceURL() + '/qcb/download-report/' + gdxServiceLogId,
        {
          responseType: 'blob',
        }
      )
  ...
  reportAudit(
    @InterceptParam(reportAuditCriteriaInterceptor.send)
    criteria: ReportAuditCriteria
  ): Observable<Pagination<ReportAuditResult[]>> {
    return this.http.post<Pagination<ReportAuditResult[]>>(
      this._getGDXServiceURL() + '/report/audit',
      criteria
    );
  }
  ...
  exportReportAudit(@InterceptParam() criteria: ReportAuditCriteria) {
    return this.http
      .post(this._getGDXServiceURL() + '/report/audit/export', criteria, {
        responseType: 'blob',
      })
      .pipe(
        map(
