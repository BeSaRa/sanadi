...
export class InboxService {
  ...
  sendTaskToMultiple(taskId: string, info: {
    taskName: string,
    comment: string,
    departments?: number[],
    users?: number[]
  }, service: BaseGenericEService<any>): Observable<boolean> {
    return this.startTaskToMultiple(taskId, info, service);
  }

  startTaskToMultiple(taskId: string, info: {
    taskName: string,
    departments?: number[],
    users?: number[]
  }, service: BaseGenericEService<any>): Observable<boolean> {
  ...
  private openSendToSingleDialog(taskId: string,
    sendToResponse: WFResponseType,
    service: BaseGenericEService<any>,
    claimBefore: boolean = false,
    ...
    return this.dialog.show(SendToSingleComponent,
      {
        inboxService: this,
        taskId,
        sendToResponse,
  ...
  getAskWFResponseByCaseType(caseType: number): WFResponseType {
    let servicesMap = {
      ...
      [CaseTypes.FINAL_EXTERNAL_OFFICE_APPROVAL]: WFResponseType.FINAL_EXTERNAL_OFFICE_SEND_TO_MULTI_DEPARTMENTS,
      [CaseTypes.PARTNER_APPROVAL]: WFResponseType.PARTNER_APPROVAL_SEND_TO_MULTI_DEPARTMENTS,
      ...
      [CaseTypes.FINANCIAL_ANALYSIS]: WFResponseType.FINANCIAL_ANALYSIS_SEND_TO_MULTI_DEPARTMENT,
      [CaseTypes.PENALTIES_AND_VIOLATIONS]: WFResponseType.PENALTIES_VIOLATIONS_SEND_TO_MULTI_DEPARTMENT,
  ...
  sendToSingleDepartment(taskId: string, caseType: number, taskName: WFResponseType, claimBefore: boolean = false, task?: QueryResult | CaseModel<any, any>): DialogRef {
    let service = this.getService(caseType);

    return this.openSendToSingleDialog(taskId, taskName, service, claimBefore, task, null);
  }
  ...
  @CastResponse(() => QueryResultSet)
  loadUserInboxByDomainName(domainName: string): Observable<QueryResultSet> {
    return this.http.get<QueryResultSet>(this.urlService.URLS.USER_INBOX + '/' + domainName);
  }

  @CastResponse(() => QueryResultSet)
  reassignBulk(queryResults: Partial<QueryResult>[], domainName: string): Observable<QueryResultSet> {
    return this.http.post<QueryResultSet>(this.urlService.URLS.REASSIGN_BULK, this.prepareQueryResults(queryResults), {
