...
export class FinalExternalOfficeApproval extends LicenseApprovalModel<FinalExternalOfficeApprovalService, FinalExternalOfficeApproval> implements IAuditModelProperties<FinalExternalOfficeApproval> {
  ...
  getBasicInfoValuesWithLabels(): { [key: string]: ControlValueLabelLangKey } {
    return {
      ...
      oldLicenseSerial: {langKey: 'serial_number', value: this.oldLicenseSerial, skipAuditComparison: true},
      ...
      countries: {langKey: 'states', value: this.countries, comparisonValue: (this.countries ?? []).sort().join(',')}
    };
  }
  ...
  getFormFields(control: boolean = false): any {
    ...
    return {
      ...
      description: control ? [values.description, [CustomValidators.maxLength(CustomValidators.defaultLengths.EXPLANATIONS)]] : values.description,
  ...
  getAdminResultByProperty(property: keyof FinalExternalOfficeApproval): AdminResult {
    ...
    switch (property) {
      ...
      case 'establishmentDate':
        ...
        break;
      case 'countries':
        const countriesValue = this.countriesInfo.filter(x => this.countries.includes(x.id!)).map(x => x.getName()).join(', ');
